...
#-------------------------------------------------------------------------------
# What: This program will compute the minor allele frequency from filtered CGS data and splits it into regions.
#		
#-------------------------------------------------------------------------------

my $usage = "filtered_split_compute_minor_alleles_All26.noXTR.pl";
...
my $CGS_nonPAR_noXTR_input = "/Users/wilsonsayres_lab_admin/Projects/PAR/BrotmanCotter/PAR_Project/Codes_04/01_CompleteGenomics/All26.females/Output_Files/split.CGS.All26.nonPAR.NOXTR.txt";
...
my $output_file_nonPAR_noXTR = "$output_dir"."$population"."_nonPAR_noXTR_minor_alleles.txt";
...
open(OUT_nonPARnoXTR, ">$output_file_nonPAR_noXTR");
...
open (CG_nonPARnoXTR, "<$CGS_nonPAR_noXTR_input") or die "Cannot open $CGS_nonPAR_noXTR_input:$!\n";
...
while (my $cg_nonPARnoXTR = <CG_nonPARnoXTR>){
		
	if ($cg_nonPARnoXTR =~ /^$|NA/) {next;}
	
	chomp($cg_nonPARnoXTR);	#get rid of newline character
	@info =  split('\s', $cg_nonPARnoXTR);
...
# COMPUTE MINOR ALLELE FREQUENCY (MIGHT BE REF OR SNP) ------------------------------------------
	...
	print OUT_nonPARnoXTR "$position\t$pop_minor\n";	
}
close(OUT_nonPARnoXTR);
close(CG_nonPARnoXTR);
print "nonPar_noXTR is done\n";

# nonPAR_nonXTR minor alleles are counted and recorded -------------------------------------------------------------------------------------------------
