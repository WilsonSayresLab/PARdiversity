...
my $usage = "filtered_split_make_freq_tables_All26_nonPAR.pl";
...
my $input_nonPAR = "$input_dir"."$population"."_nonPAR_minor_alleles_minus_low.txt";
...
while (my $nonPAR = <nonPAR>){
	
	if ($nonPAR =~ /position/){
		next;
	}

	chomp($nonPAR);	#get rid of newline character
	@info =  split('\s', $nonPAR);
